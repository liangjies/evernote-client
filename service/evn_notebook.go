...
func DeleteNotebook(id uint, uid uint) (err error) {
	err = global.DB.Where("notebook_id = ? AND del_flag=0", id).First(&model.EvnNote{}).Error
	if err != nil {
		var notebook model.EvnNotebook
		err = global.DB.Where("id = ? AND create_by= ?", id, uid).Delete(&notebook).Error
		return err
	} else {
...
func UpdateNotebook(n model.EvnNotebook, id uint, uid uint) (err error) {
	var notebook model.EvnNotebook
	if !errors.Is(global.DB.Where("title = ? AND id != ? AND create_by = ?", n.Title, id, uid).First(&notebook).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
		return errors.New("笔记本已存在")
	}
	err = global.DB.Where("id = ? AND create_by = ?", id, uid).First(&notebook).Update("title", n.Title).Error
	return err
}
...
func CreateNotebook(n model.EvnNotebook, id uint) (err error) {
	var notebook model.EvnNotebook
	if !errors.Is(global.DB.Where("title = ? AND create_by = ?", n.Title, id).First(&notebook).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
		return errors.New("笔记本已存在")
	}
	n.CreateBy = id
	err = global.DB.Create(&n).Error
	return err
}
...
func GetNotebooks(id uint) (err error, list interface{}, total int64) {
	...
	db := global.DB.Model(&model.EvnNotebook{})
